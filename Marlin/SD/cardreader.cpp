...
void CardReader::lsDive(const char *prepend, SdFile parent, const char * const match/*=NULL*/
  ...
  while (parent.readDir(p, longFilename) > 0) {
    ...
    if (DIR_IS_SUBDIR(&p) && lsAction != LS_Count && lsAction != LS_GetFilename) {
      ...
      if (!filenameIsDir && (p.name[8] != 'G' || p.name[9] == '~') && p.name[8] != 'B') continue;
...
void CardReader::openAndPrintFile(const char *name) {
  ...
  char *cmd=(char*)malloc(4 + strlen(name) + 1);
...
void CardReader::removeFile(const char * const name) {
  ...
  char *dirname_start, *dirname_end;
  if (name[0] == '/') {
    dirname_start = (char*)strchr(name, '/') + 1;
    while (dirname_start != NULL) {
      dirname_end = strchr(dirname_start, '/');
      //SERIAL_ECHOPGM("start:");SERIAL_ECHOLN((int)(dirname_start - name));
      //SERIAL_ECHOPGM("end  :");SERIAL_ECHOLN((int)(dirname_end - name));
...
void CardReader::getStatus(
  ...
  extern HAL_SD_CardCIDTypeDef cid;
    
    SERIAL_PROTOCOLPGM_P(port, "CID.ManufacturerID=");
    print_hex_byte(cid.ManufacturerID);
    
    SERIAL_PROTOCOLPGM_P(port, "\nCID.OEM_AppliID=");
    print_hex_word(cid.OEM_AppliID);
    
    SERIAL_PROTOCOLPGM_P(port, "\nCID.ProdName1=");
    SERIAL_ECHO(hex_long(cid.ProdName1));
    
    SERIAL_PROTOCOLPGM_P(port, "\nCID.ProdName2=");
    print_hex_byte(cid.ProdName2);
    
    SERIAL_PROTOCOLPGM_P(port, "\nCID.ProdRev=");
    print_hex_byte(cid.ProdRev);
    
    SERIAL_PROTOCOLPGM_P(port, "\nCID.ProdSN=");
    SERIAL_ECHO(hex_long(cid.ProdSN));
    
    SERIAL_PROTOCOLPGM_P(port, "\nCID.ManufactDate=");
    print_hex_word(cid.ManufactDate);
...
void CardReader::printingHasFinished() {
...
#ifdef MALYAN_LCD
    extern void write_to_lcd(const char * const message);
  ...
  const char job_recovery_file_name[] = "recovery.bin";
  ...
  void CardReader::openJobRecoveryFile(const bool read) {
    ...
    bool ret = false;
    //while (ret==false)
      ret = jobRecoveryFile.open(&root, job_recovery_file_name, read ? O_READ : O_CREAT | O_WRITE | O_TRUNC | O_SYNC);
    return;
    // (jobRecoveryFile.isOpen()) return;
    if (!jobRecoveryFile.open(&root, job_recovery_file_name, read ? O_READ : O_CREAT | O_WRITE | O_TRUNC | O_SYNC)) {
    //if (!jobRecoveryFile.open(&root, job_recovery_file_name, read ? O_READ : O_CREAT | O_WRITE)) {
      /*SERIAL_PROTOCOLPAIR(MSG_SD_OPEN_FILE_FAIL, job_recovery_file_name);
      SERIAL_PROTOCOLCHAR('.');
      SERIAL_EOL();*/
      //extern void write_to_lcd_P(const char * const message);
      //write_to_lcd_P("{ERR:RECOVERY}");
      jobRecoveryFile.open(&root, job_recovery_file_name, read ? O_READ : O_CREAT | O_WRITE);
    }
    else
    {
  ...
  int16_t CardReader::saveJobRecoveryInfo() {
    ...
    int16_t ret = -1;
    //while (ret==-1)
        ret = jobRecoveryFile.write(&job_recovery_info, sizeof(job_recovery_info));
    //if (ret != -1) jobRecoveryFile.sync();
    /*if (ret == -1) SERIAL_PROTOCOLLNPGM("Power-loss file write failed.");
    else jobRecoveryFile.sync();*/
    ret = 0;
    //while (ret==0)
    {
        ret = jobRecoveryFile.close();
    }
    return ret;
  }
