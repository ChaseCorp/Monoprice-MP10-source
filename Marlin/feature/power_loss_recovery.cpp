  ...
  if (card.cardOK) {
    ...
    if (card.jobRecoverFileExists()) {
      ...
      //if (job_recovery_info.valid_head && job_recovery_info.valid_head == job_recovery_info.valid_foot) 
      {
        ...
        strcpy_P(job_recovery_commands[ind++], PSTR("G28 X\nG28 Y#"));
        ...
        char str_3[16], str_4[16];
        //dtostrf(job_recovery_info.current_position[Z_AXIS]+5.20f, 1, 3, str_1);
        ...
        //enqueue_and_echo_command("G1 Z2");
        //enqueue_and_echo_command("G28 R0 X Y");
        
        sprintf_P(job_recovery_commands[ind++], PSTR("G92 Z%s E%s#"), str_1, str_2); // Current Z + 2 and E
        
        /*dtostrf(job_recovery_info.current_position[X_AXIS], 1, 3, str_3);
        dtostrf(job_recovery_info.current_position[Y_AXIS], 1, 3, str_4);
        dtostrf(job_recovery_info.current_position[Z_AXIS], 1, 3, str_1);
        sprintf_P(job_recovery_commands[ind++], PSTR("G1 X%s Y%s Z%s#"), str_3,str_4,str_1);*/
        //enqueue_and_echo_command(job_recovery_commands);
        ...
        write_to_lcd("{PRINTFILE:");
        write_to_lcd(job_recovery_info.sd_filename);
        ...
        extern void lcd_sdcard_recover_job();
...
void save_job_recovery_info() {
  if (card.sdprinting==false) return;
  ...
  if (
    ...
    if (f_mount(&SDFatFS, SDPath, 0) == FR_OK)
    {
        ...
        if (f_open(&SDFile, "RECOVERY.BIN", FA_CREATE_ALWAYS | FA_WRITE) == FR_OK) {
            if (0==f_write(&SDFile,&job_recovery_info, sizeof(job_recovery_info),&w))
            {
              f_close(&SDFile);
            }
        }
