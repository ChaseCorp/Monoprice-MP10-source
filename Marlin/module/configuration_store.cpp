...
void MarlinSettings::postprocess() {
  ...
  bool MarlinSettings::_load(
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      float tmp1[XYZE_N], tmp2[XYZE_N];
      uint32_t tmp3[XYZE_N];
      /*float* tmp1=(float*)malloc((XYZ + esteppers)*sizeof(float));
      float* tmp2=(float*)malloc((XYZ + esteppers)*sizeof(float));
      uint32_t* tmp3=(uint32_t*)malloc((XYZ + esteppers)*sizeof(float));
      for (uint8_t i=0;i<XYZ + esteppers;i++) EEPROM_READ(tmp1[i]);
      for (uint8_t i=0;i<XYZ + esteppers;i++) EEPROM_READ(tmp2[i]);
      for (uint8_t i=0;i<XYZ + esteppers;i++) EEPROM_READ(tmp3[i]);*/
      EEPROM_READ(tmp1);
      EEPROM_READ(tmp2);
      EEPROM_READ(tmp3);
  ...
  bool MarlinSettings::load(
    ...
    void MarlinSettings::store_mesh(const int8_t slot) {
        ...
        uint16_t crc = 0;
        int pos = 0x200;//meshes_end - (slot + 1) * sizeof(ubl.z_values);
