...
void HAL_timer_start(TIM_TypeDef* timer_num, const uint32_t frequency) {
    ...
    if (timer_num==STEP_TIMER_NUM)
    {
        ...
        TIM_TimeBaseStructure.TIM_Prescaler = 42-1;//(SystemCoreClock/20000000)-1;//42-1;   //2Mhz
    ...
    else
    {
        ...
        TIM_TimeBaseStructure.TIM_Prescaler = 84-1;//(SystemCoreClock/10000000)-1;//84-1;//1Mhz
        TIM_TimeBaseStructure.TIM_ClockDivision = 0;
        TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
        TIM_TimeBaseStructure.TIM_RepetitionCounter = 0;
...
extern "C" void STEPPER_IRQHandler()
{
    //TIM_ClearITPendingBit(STEP_TIMER_NUM, TIM_IT_UPDATE);
    stepTC_Handler();
}

extern "C" void TEMP_IRQHandler()
{
    //TIM_ClearITPendingBit(TEMP_TIMER_NUM, TIM_IT_UPDATE);
    tempTC_Handler();
}
