...
void GcodeSuite::M115() {
  ...
  if (parser.seenval('S'))
  {
    bool e = parser.boolval('S');
    extern bool filament_sensor;
    filament_sensor=e;
  }
  SERIAL_PROTOCOLLNPGM_P(port, MSG_M115_REPORT);

  SERIAL_PROTOCOLPGM_P(port, "Uptime(ms): ");
  SERIAL_PROTOCOLLN(millis());
  
#ifdef MALYAN_LCD
  SERIAL_PROTOCOLPGM_P(port, "SDIO error: ");
  extern int32_t sd_err_cnt;
  SERIAL_PROTOCOLLN(sd_err_cnt);
#endif
  
  extern int32_t filament_sig;
  SERIAL_PROTOCOLPGM_P(port, "Filament Sensor: ");
  SERIAL_PROTOCOLLN(filament_sig);
  
  SERIAL_PROTOCOLPGM_P(port, "CPU temperature: ");
  extern uint16_t cpuTemp;
  SERIAL_PROTOCOLLN(cpuTemp);
